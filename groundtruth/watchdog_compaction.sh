#!/bin/bash
# watchdog: wait for CAffNet arm2b logbook, score + compare vs original arm2,
# write COMPACTION_DECISION.txt. Notifies when done.
set +e
...
echo "watchdog start $(date)" > "$DEC"

for i in $(seq 1 50); do  # up to 50 min
  if [ -f "$RUN/20hdQQQrA4_arm2b/logbook.md" ]; then
    echo "logbook found at $(date)" >> "$DEC"
    cd "$REPO" && $PY score_run.py 20hdQQQrA4_arm2b >> "$DEC" 2>&1
    echo "=== compare ===" >> "$DEC"
...
echo "TIMEOUT: logbook not found in 50 min" >> "$DEC"
exit 1
