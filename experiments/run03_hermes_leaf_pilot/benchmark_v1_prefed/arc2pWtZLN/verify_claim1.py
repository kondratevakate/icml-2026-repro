...
def full_energy(u, z):
    vec = np.vstack([u.reshape(-1, 1), z.reshape(-1, 1)])
    return float(vec.T @ Lstar @ vec)
...
# ---- MUTATION: wrong coupling block (flipped sign) ----
B1 = cb.build_B1()
X_wrong = np.block([+GAMMAS[0] * B1, np.zeros((n0, X.shape[1] - B1.shape[1]))])
...
result = {
    ...
    "mutation": {
        "description": "Flip sign of coupling block X -> X_wrong; recompute S_wrong = A - X_wrong C^{-1} X_wrong^T.",
        "energy_identity_breaks_max_abs_err": mut_energy_err,
        "breaks": mut_energy_err > 1e-6,
    },
    "verdict": verdict,
